...
Accurate estimation of the null distribution is essential for assessing statistical significance. However, for most network-based statistics, the null distribution is difficult to derive analytically. **RSNet** addresses this challenge by implementing **resampling-based null distribution generation** via **permutation** and **bootstrap** methods. These procedures are implemented in the `null_ggm()` function.
...
In the following example, we generate two null distributions, one using permutation and one using bootstrap, each with five iterations. The output from `null_ggm()` can be used as the input of the `diff_centrality()` function in the following section. 
