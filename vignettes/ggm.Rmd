...
These procedures are implemented in the function `ensemble_ggm()`, which leverages structure inference algorithms from the `SILGGM` [package](https://cran.r-project.org/web/packages/SILGGM/index.html). The resampling module supports parallel computing, enabling efficient large-scale network inference and ensuring scalability for high-dimensional datasets.
